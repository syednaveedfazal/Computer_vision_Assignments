...
class MOG():
    ...
    def updateParam(self, img, BG_pivot): #finish this function
        ...
        # Update Means and Variances for Matched Gaussians
        ...
        # Handle Unmatched Pixels (Replace lowest priority Gaussian)
        no_match = ~has_match
        ...
        # Sort Gaussians by Fitness (weight / sigma)
...
for i in range(1, 3+1):#display first 3 labeled foreground images
    ...
    plt.title(f'Frame {i} Foreground')
    plt.axis('off') # Hides the default ticks and labels

    ax = plt.gca() 
    rect = plt.Rectangle((0, 0), 1, 1, transform=ax.transAxes, linewidth=4, edgecolor='black', facecolor='none')
    ax.add_patch(rect)
